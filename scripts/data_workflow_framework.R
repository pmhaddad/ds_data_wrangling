...
links <- read.xlsx(xlsxFile = '../data/data_wrangling.xlsx', sheet = 'data_workflow_framework_links')
...
nodes$name[c(1,10,18)] <- c("RAW STAGE", "REFINED STAGE", "PRODUCTION STAGE")
links$source[c(1,19,32)] <- c("RAW STAGE", "REFINED STAGE", "PRODUCTION STAGE")

# Make the Network
sn <- sankeyNetwork(Links = links, Nodes = nodes,
                    Source = "id_source", Target = "id_target", LinkGroup = "source",
                    Value = 'value', NodeID = "name",
                    sinksRight = FALSE, fontSize = 13, fontFamily = 'Helvetica')
...
saveNetwork(sn, 'data_workflow_framework.html', selfcontained = TRUE)
